...
export const EventList = (props) => {
    ...
    const formatDateTime = (date, time) => {
        return moment(`${date} ${time}`, 'YYYY-MM-DD HH:mm:s').format('LLL')
    }
                    ...
                    props.events.map(event => {
                        ...
                        return (
                            <section className="event" key={`event--${event.id}`}>
                                ...
                                <div className="event__date">Date: {formatDateTime(event.date, event.time)}</div>
                                <div className="event__date">Number of attendees: {event.attendees_count}</div>
                                <div className='btn-container'>
                                    {
                                        event.joined
                                            ...
                                            :
                                            <button className='btn join-btn' onClick={() => handleJoinEvent(event.id)}>Join event</button>
                                    }
